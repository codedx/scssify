...
function scssify(file, content, config, stream, done) {
  ...
  sass.render(sassOpts, function (err, firstResult) {
    ...
    function generateModule(result) {
      ...
      emitDependencies(stream, firstResult.stats.includedFiles, sassOpts.file)
      stream.push(out)
      done()
    }
  })
 ...
 * @param {string[]} deps
 * @param startPath Reference to the original file, in order to exclude it from being emitted
 */
function emitDependencies(stream, deps, startPath) {
  startPath = path.normalize(startPath)
  for (let index = 0; index < deps.length; ++index) {
    const dep = deps[index]
    if (path.normalize(dep) !== startPath) {
